<div class="main-content">
    <div class="main-content-inner">
        ...
        <div class="our-brands" style="margin-left: 250px;">
            <div class="our-brands-container">
                <h2>OUR SUCCESS STORIES &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </h2>

                <?php if ($this->session->userdata('type') == 'admin') { ?>
                            <h2><a
                        href="<?php echo site_url() ?>/admin/Successstories/add"> <button id="add-btn"
                            class="btn btn-info">Add New Couple</button></a></h2>
                            <h2><a
                        href="<?php echo site_url() ?>/admin/Successstories/advertisement"> <button id="add-btn"
                            class="btn btn-secondary">Add advertisement Videos</button></a></h2>
                            <?php } ?>
            ...
            foreach ($record as $value) { ?>
                    <div class="col-sm-4 text-center p-2">
                        <h1
                            style="margin-bottom: 15px; font-family: cursive; font-size: 22px; text-transform: uppercase; text-shadow: 2px 2px 4px #888888; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
                            <?php echo $value['title'] ?></h1>
                        <div class="success_stories">
                            <img src="<?php echo site_url(); ?>images/successstories/<?php echo $value['image']; ?>"
                                alt="img">
                            <div class="image-overlay">
                            <?php if ($this->session->userdata('type') == 'admin') { ?>
                                <div class="button-container" ; style="margin-top:20px;">
                                    <button class="edit-btn"><a
                                            href="<?php echo site_url() ?>/admin/Successstories/edit/<?php echo $value['id'] ?>">Edit</a></button>
                                    <button class="delete-btn"><a
                                            href="<?php echo site_url() ?>/admin/Successstories/delete/<?php echo $value['id'] ?>">Delete</a></button>
                                    <button class="delete-active"><a
                                            href="<?php echo site_url() ?>/admin/Successstories/status/<?php echo $value['id'] ?>/<?php echo $value['status'] ?>"><?php echo ($value['status'] == 0)? 'Active' : 'Inactive' ?></a></button>
                                </div>
                                <?php }?>
                            </div>
                            <div>
                ...
                $i++;
                if ($i % 3 == 0) {
                    echo '</div><div class="row our-brands-row">';
                }
            } ?>
                </div>
            </div>
                    ...
                    <form method="post" action="<?php echo ADMIN_URL; ?>successstories/delete_selected">
                        <div class="clearfix">
                            ...
                            <div class="pull-right">
                                ...
                                <button class="btn btn-danger" type="submit" onClick="return delete_selected_fields();"
                            ...
                            <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="center">
                                            <label class="pos-rel">
                                                <input type="checkbox" name="selected_ids[]" id="selected_ids[]"
                                                    value="<?php echo $row['id']; ?>" class="ace" />
                                                <span class="lbl"></span>
                                            </label>
                                        </td>
                                        <td><?php echo $i; ?></td>
                                        ...
                                        <td><img style="width:200px;height:150px;"
                                                src="<?php echo site_url();?>images/successstories/<?php echo $row['image'];?>" />
                                        </td>
                                        <td><?php $cdata4 = preg_replace('/<[^>]*>/', '', $row['content']);
                                            echo substr(stripcslashes(str_replace("\n", "", $cdata4)), 0, 50); ?></td>
                                        <td><span
                                                <?php if($row['status']== 1){ echo " class='label label-success arrowed-in arrowed-in-right'"; } elseif($row['status']== 0){ echo "class='label label-danger arrowed'"; } ?>><a
                                                    href="JavaScript:if(confirm('Are you sure want to Chanage the Status?')==true){window.location='<?php echo ADMIN_URL;?>successstories/status/<?php echo $row['id']?>/<?php echo $row['status'];?>';}"
                                                    style="color:#FFF; text-decoration:none;"><?php if($row['status']== 1){ echo "Active"; } elseif($row['status']== 0){ echo "De-Active"; } ?></a></span>
                                        </td>
                                        ...
                                        <td>
                                            <div class="hidden-sm hidden-xs action-buttons">
                                                ...
                                                <a class="green"
                                                    href="<?php echo ADMIN_URL; ?>successstories/edit/<?php echo $row['id']; ?>">
                                                    <i class="ace-icon fa fa-pencil bigger-130"></i>
                                                </a>

                                                <a class="red"
                                                    href="JavaScript:if(confirm('Confirm Delete?')==true){window.location='<?php echo ADMIN_URL; ?>successstories/delete/<?php echo $row['id']; ?>';}">
                                            ...
                                            <div class="hidden-md hidden-lg">
                                                <div class="inline pos-rel">
                                                    ...
                                                    <ul
                                                        ...
                                                        <li>
                                                            <a href="<?php echo ADMIN_URL; ?>successstories/edit/<?php echo $row['id']; ?>"
                                                        ...
                                                        <li>
                                                            <a href="JavaScript:if(confirm('Confirm Delete?')==true){window.location='<?php echo ADMIN_URL; ?>successstories/delete/<?php echo $row['id']; ?>';}"
...
jQuery(function($) {
    ...
    $('#simple-table > thead > tr > th input[type=checkbox]').eq(0).on('click', function() {
        ...
        $(this).closest('table').find('tbody > tr').each(function() {
            var row = this;
            if (th_checked)
                $(row).addClass(active_class).find('input[type=checkbox]').eq(0).prop('checked',
                    true);
            else
                $(row).removeClass(active_class).find('input[type=checkbox]').eq(0).prop(
                    'checked', false);
        });
    });
