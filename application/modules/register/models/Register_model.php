...
class Register_model extends CI_Model {
    ...
    public function add_primaryinfo($code) {
		...
		$set_data = array('email' => mysqli_real_escape_string($this->get_mysqli(), $this->input->post('email', TRUE)), 'password' => md5(mysqli_real_escape_string($this->get_mysqli(), $this->input->post('password', TRUE))), 'mobile' => mysqli_real_escape_string($this->get_mysqli(), $this->input->post('mobile', TRUE)), 'first_name' => mysqli_real_escape_string($this->get_mysqli(), ucwords($this->input->post('first_name', TRUE))), 'last_name' => mysqli_real_escape_string($this->get_mysqli(), $this->input->post('last_name', TRUE)), 'gender' => mysqli_real_escape_string($this->get_mysqli(), $this->input->post('gender', TRUE)), 'date' => mysqli_real_escape_string($this->get_mysqli(), $this->input->post('date', TRUE)), 'month' => mysqli_real_escape_string($this->get_mysqli(), $this->input->post('month', TRUE)), 'year' => mysqli_real_escape_string($this->get_mysqli(), $this->input->post('year', TRUE)), 'profile_id' => 'KV' . $profile_id . $last_id, 'living_in' => mysqli_real_escape_string($this->get_mysqli(), $this->input->post('country', TRUE)), 'age' => $age, 'status' => 1, 'registered_on' => date('Y-m-d H:i:s'), 'activation_code' => $code);
        ...
        if ($this->session->userdata('staff_id') || $this->session->userdata('admin_id')) {
            $array = array('user_fkid' => $insert_id, 'rm_fkid' => ($this->session->userdata('staff_id')) ? $this->session->userdata('staff_id') : $this->session->userdata('admin_id'), 'rm_created_date' => date('Y-m-d H:i:s'));
