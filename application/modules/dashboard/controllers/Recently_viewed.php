...
class Recently_viewed extends CI_Controller {
	...
	public $listPage     = 'recently_viewed';
...
public function __construct() {
		...
		$this->session->set_userdata('page_name','recently_viewed');
       ...
       $data['contact']=$this->my_account_model->contact($this->session->userdata('user_id'));
		...
		$data['conf_profiles_for_regular'] = $this->my_account_model->getConfidentialProfilesForRegular($res, $res1, $res2);
				...
				//print_r($data['conf_profiles_for_regular']);exit;
				
				$uriSegment = 4; 
				 
				// Get record count 
				$conditions['returnType'] = 'count'; 
				//$totalRec = $this->my_account_model->getNewProfiles($res, $res1, $res2, $conditions);
				  $totalRec = $this->my_account_model->getProfilesViewedByUser($this->session->userdata('user_id'), $conditions); 
				 
				// Pagination configuration 
				$config['base_url']    = base_url().'dashboard/recently_viewed/index/'; 
			...
			//	print_r($conditions);
				...
				$data['viewed_profiles'] = $this->my_account_model->getProfilesViewedByUser($this->session->userdata('user_id'), $conditions);
				
				
			//	print_r($data['viewed_profiles']);//exit;
