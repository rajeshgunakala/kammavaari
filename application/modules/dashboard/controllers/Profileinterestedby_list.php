...
class Profileinterestedby_list extends CI_Controller {
...
public function __construct() {
       ...
       $data['get_my_contactlist'] = $this->my_model->profileinterestedby_list();
      ...
      $data['recently_viewed']=$this->my_account_model->recently_viewed($this->session->userdata('user_id'));
       ...
       $data['contact']=$this->my_account_model->contact($this->session->userdata('user_id'));
        ...
        $data = $conditions = array(); 
				...
				$totalRec  = $this->my_account_model->profile_intrest_by_count($this->session->userdata('user_id'),$conditions);
				$config['base_url']    = base_url().'dashboard/Profileinterestedby_list/index/'; 
				...
				$data['total_decision_pending_profiles'] = $totalRec;
				$data['decision_pending_profiles'] = $this->my_account_model->profile_intrest_by_count($this->session->userdata('user_id'),$conditions);;
        
                $this->load->view($this->headerPage,$data);
                $this->load->view('Profileinterestedby_list',$data);
