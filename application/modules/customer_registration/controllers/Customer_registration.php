...
class Customer_registration extends CI_Controller {
    public $headerPage = '../../views/header_latest';
    public $footerPage = '../../views/footer_latest';
    public $customerRegistration = 'customer_registration';
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model('Customer_registration_model', 'customerRegModel');
    ...
    public function get_email_availability() {
			$email = $this->input->post('email');
            $encryptEmail = $this->Encrypt_model->hashInput($email);
			$results = $this->customerRegModel->check_email_exist($encryptEmail);
			echo $results;
			if ($results) {
				return true;
            }else{
	...
	public function get_number_availability() {
            ...
            $encryMobile = $this->Encrypt_model->hashInput($number);
			$results = $this->customerRegModel->check_number_exist($encryMobile);
			echo $results;
			if ($results) {
				return true;
            }else{
    ...
    public function index() {
        ...
        if ($this->input->post('submit') != '') {
            ...
            // if ($status["success"] != 1) {
            //     $this->session->set_flashdata('msg_failure', 'Something went wrong, please try again.');
            //     redirect(site_url() . "registration");
            // }
            $exit_details = $this->customerRegModel->exit_details($email, $mobile);
            //log_message('debug',"output emial and mobile data".$exit_details['email']);
           // print_r($exit_details);exit;
            if ($exit_details['rows'] == 0) {
				$set = '123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
				$code = substr(str_shuffle($set), 0, 12);
                $insert_id = $this->customerRegModel->add_primaryinfo($code);
                
				$updateBatteryProfile = $this->customerRegModel->update_battery_primary($insert_id);
                $this->session->set_userdata('user_reg_id', $insert_id);
                if ($insert_id) {
					//Email verification
				// 	$encUserId = $this->encrypt->encode($insert_id);
					$emailVerificationUrl = base_url()."registration/activate/".$insert_id."/".$code;
                    ...
                    redirect(site_url() . "registration/religion");
                } else {
        ...
        $data = $this->customerRegModel->get_last_id();
        $data['id'] = $data['id'];
        $data['countries'] = $this->customerRegModel->get_all_countries();
		$data['religions'] = $this->customerRegModel->get_all_religions();
        $data['mother_tounges'] = $this->customerRegModel->get_all_mothertounges();
        $data['rmdata'] = $this->customerRegModel->getrmdata();
        $data['castes'] = $this->customerRegModel->get_all_castes();
        $this->load->view($this->headerPage, $header);
        $this->load->view($this->customerRegistration, $data);
	...
	public function activate() {
		...
		//fetch user details
		$user = $this->customerRegModel->getUserDetailsById($decUserId);
		...
		if($user['activation_code'] == $code) {
			...
			$data['is_Everified'] = 1;
			$data['status'] = 1;
			$query = $this->customerRegModel->activateUser($data, $decUserId);
			if($query) {
	...
	public function redirectAfterEmailVerified($id) {
		$data = $this->customerRegModel->getUserDetailsById($id);
    ...
    public function religion() {
        ...
        if ($this->input->post('submit') != '') {
            $result = $this->customerRegModel->add_religioninfo();
            if ($result) {
                $updateBatteryProfile = $this->customerRegModel->update_battery_religion($regUserId);
                redirect(site_url() . "registration/profession");
            } else {
                $this->session->set_flashdata('msg_succ', 'Something Went Wrong');
                redirect(site_url() . "registration/religion");
            }
        }
        $data['users_info'] = $this->customerRegModel->getsessiondetails($regUserId);
        $data['heights'] = $this->customerRegModel->get_all_heights();
        $data['religions'] = $this->customerRegModel->get_all_religions();
        $data['mother_tounges'] = $this->customerRegModel->get_all_mothertounges();
        $data['castes'] = $this->customerRegModel->get_all_castes();
        $data['zodic_signs'] = $this->customerRegModel->get_all_zodic_signs();
        $data['stars'] = $this->customerRegModel->get_all_stars();
    ...
    public function profession() {
        ...
        if ($this->input->post('submit') != '') {
            $result = $this->customerRegModel->add_profession_info();
            if ($result) {
                $updateBatteryProfile = $this->customerRegModel->update_battery_profession($regUserId);
                redirect(site_url() . "registration/property");
            } else {
                $this->session->set_flashdata('msg_succ', 'Something Went Wrong');
                redirect(site_url() . "registration/profession");
            }
        ...
        $data['users_info'] = $this->customerRegModel->getsessiondetails($regUserId);
        $data['countries'] = $this->customerRegModel->get_all_countries();
        $data['professions'] = $this->customerRegModel->getallprofessions();
        $data['heighsteducation'] = $this->customerRegModel->get_heighsteducations();
		//echo "<pre>";print_r($data['heighsteducation']);exit;
        $this->load->view($this->headerPage, $data);
    ...
    public function property() {
        ...
        $data['profile_complete'] = $this->dashboard_model->get_profile_complete($regUserId);
        if ($this->input->post('submit') != '') {
			$result = $this->customerRegModel->add_parents_info();
		//	$result = $this->customerRegModel->add_siblingsinfo();
            $result = $this->customerRegModel->add_property_info();
            if ($result) {
                $updateBatteryProfile = $this->customerRegModel->update_battery_profession($regUserId);
                redirect(site_url() . "registration/upload");
            }
        } 
		$data['property_values'] = $this->customerRegModel->getPropertyValueRanges();
		$this->load->view($this->headerPage, $data);
    ...
    public function parents() {
        ...
        if ($this->input->post('submit') != '') {
            $result = $this->customerRegModel->add_parents_info();
            if ($result) {
                $updateBatteryProfile = $this->customerRegModel->update_battery_parents($regUserId);
                redirect(site_url() . "registration/siblings");
            } else {
                $this->session->set_flashdata('msg_succ', 'Something Went Wrong');
                redirect(site_url() . "registration/parents");
            }
        }
        $data['users_info'] = $this->customerRegModel->getsessiondetails($regUserId);
        $data['states'] = $this->customerRegModel->get_all_states();
        $this->load->view($this->headerPage, $data);
    ...
    public function siblings() {
        ...
        if ($this->input->post('submit') != '') {
            $result = $this->customerRegModel->add_siblingsinfo();
            if ($result) {
                $updateBatteryProfile = $this->customerRegModel->update_battery_siblings($regUserId);
                redirect(site_url() . "registration/upload");
            } else {
                $this->session->set_flashdata('msg_succ', 'Something Went Wrong');
                redirect(site_url() . "registration/siblings");
    ...
    public function upload() {
        ...
        if ($this->input->post('submit_image') && !empty($_FILES['images']['name'])) {
            ...
            if (!empty($uploadData)) {
                $insert = $this->customerRegModel->add_images($uploadData);
                if ($insert) {
                    $updateBatteryProfile = $this->customerRegModel->update_battery_images($user_id);
                    redirect(site_url() . "registration/success");
                } else {
                    $this->session->set_flashdata('msg_succ', 'Something Went Wrong');
                    redirect(site_url() . "registration/upload");
                }
    ...
    public function getsubcastes() {
        ...
        $castes = $this->customerRegModel->get_all_subcastes_ajax($caste);
    ...
    public function getdistricts() {
        ...
        $districts = $this->customerRegModel->get_all_districts_ajax($state_id);
    ...
    public function getstates() {
        ...
        $states = $this->customerRegModel->get_all_states_ajax($country_id);
    ...
    public function getdegree() {
        ...
        $degrees = $this->customerRegModel->get_all_degrees_ajax($h_id);
    ...
    public function getspecialization() {
        $h_id = $this->input->post('valu');
        $specialization = $this->customerRegModel->get_all_specialization_ajax($h_id);
