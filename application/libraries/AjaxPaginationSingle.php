...
class AjaxPaginationSingle{
	...
	function create_links(){
		...
		if ($num_pages == 1){
            		//$info = '<div class="showing_no_pages">Showing : ' . $this->total_rows."</div>";
			//return $info;
		}
		...
		if ($this->show_count){
			...
			$info = '<div class="showing_no_pages">Total Records: <span class="cur_page">' . ( $curr_offset + 1 ).'</span>';
	...
	function getAJAXlink($count, $text) {
		...
		if($this->total_rows == $pageCount || $pageCount == "-1"){
			return '<span class="arrow_link">'.$text.'</span>';
		} else if($pageCount >= 0) {
			return '<a class="arrow_link" href="javascript:void(0);"' . $this->anchor_class . ' onclick="'.$this->link_func.'('.$pageCount.')">'. $text .'</a>';
		}
	}
